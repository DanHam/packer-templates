...
function Install-WindowsUpdates() {
    ...
    for($i=0; $i -lt $UpdatesToInstall.Count; $i++) {
        $Returned = $InstallationResult.GetUpdateResult($i).ResultCode
        LogWrite "Update: $($UpdatesToInstall.Item($i).Title)"
        LogWrite "Result: $($OperationResultCodes[$Returned])"
    }
...
function Check-WindowsUpdates() {
    ...
    if ($SearchResult.Updates.Count -ne 0) {
        LogWrite "Number of applicable updates: $($SearchResult.Updates.Count)"
