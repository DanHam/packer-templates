...
# Repair Amazons effort with rc.local files and symlinks
#
RCLOCAL="/etc/rc.d/rc.local"
# Remove and recreate the original file
rm -f ${RCLOCAL}
cat <<EOF > ${RCLOCAL}
#!/bin/bash
...
EOF
# Now ensure the correct permissions are set
chmod 0644 ${RCLOCAL}

# Remove any effort put in the place of /etc/rc.local
rm -f /etc/rc.local
# ... and return it to the symlink it ought to be
ln -s ${RCLOCAL} /etc/rc.local
